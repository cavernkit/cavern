<script lang="ts" generics="T extends string | boolean | number | null = string">
    ...
    type RadioProps = {
        value?: T;
        options: T extends string ? Array<Option<T> | string> : Array<Option<T>>;
        disabled?: boolean;
        children?: never;
    };
    type ChildProps = {
    ...
    type Props = BaseProps & (RadioProps | ChildProps);
    ...
    const radios = $derived(options?.map((x) => (typeof x === "string" ? { label: x, value: x } : x))) as Option<T>[];
</script>
